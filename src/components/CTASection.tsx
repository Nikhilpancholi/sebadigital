import { ArrowRight, Sparkles } from 'lucide-react';

export function CTASection() {
  return (
    <section className="bg-gradient-to-br from-blue-600 via-blue-700 to-blue-900 py-20 relative overflow-hidden">
      <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-30"></div>
      
      <div className="relative max-w-4xl mx-auto px-6 text-center">
        <Sparkles className="w-12 h-12 text-yellow-300 mx-auto mb-6" />
        <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">
          Ready to Upgrade Your Tech?
        </h2>
        <p className="text-xl text-blue-100 mb-8">
          Browse our latest collection of premium electronics and find exactly what you need.
        </p>
        <button className="bg-white hover:bg-gray-100 text-blue-600 px-8 py-4 rounded-lg font-semibold flex items-center gap-2 mx-auto">
          Shop Now
          <ArrowRight className="w-5 h-5" />
        </button>
      </div>
    </section>
  );
}
